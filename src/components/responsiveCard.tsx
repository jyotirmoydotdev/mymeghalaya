...
const ResponsiveCard = ({ i, url, imgUrl, hover=true, imgBlurDataUrl, name, alt, des, icon, imgMsg, className, grid }: { i: number, imgBlurDataUrl?:string, alt?:string,hover?:boolean, url: string, imgUrl: string, name?: string, des: string, icon?: ReactNode, imgMsg?: string, className?:string, grid?: boolean }) => {
    ...
    <Link href={url} key={i}  className='transform active:scale-95 transition-all group' onClick={handleClick}>
      <div className={cn("grid grid-cols-1 sm:grid-cols-1 rounded-xl text-card-foreground w-[169px] sm:w-full",className)}>
        <div className="relative flex items-center justify-start sm:justify-center h-24 sm:h-52">
          <DirectionAwareHover hover={hover} imageUrl={imgUrl} alt={alt} imgBlurDataUrl={imgBlurDataUrl} name={name as string} className='rounded-[11px] sm:rounded-xl h-24 sm:h-52'>
